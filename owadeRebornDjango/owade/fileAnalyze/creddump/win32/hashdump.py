...
def owade_get_hashes(sysaddr, samaddr):
    ...
    samaddr = HiveFileAddressSpace(samaddr)
    hashes = {}
    bootkey = get_bootkey(sysaddr)
    hbootkey = get_hbootkey(samaddr,bootkey)
    ...
    for user in get_user_keys(samaddr):
        ...
        hashes[name] = {'name':name, 'id':int(user.Name,16),
            'lmhash':lmhash.encode('hex'), 'nthash':nthash.encode('hex')}
    return hashes
#################################################
###### END OWADE CODE ADAPTATION
#################################################
