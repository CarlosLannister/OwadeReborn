...
class Extrator(Paths):
	...
	def extractFiles(self):			#Main function for extract the important files from the harddrive (only extract files from NTFS partitions)
				...
				try:
					...
					for entryObject in directoryObject:
						...
						if userDir != '.' and userDir != '..' :								#Quit . and .. directories
							print "--", userDir
							done = self.getMasterKey(myPath, userDir, filesystemObject)		#Try to get masterkey from users
							if done:
								self.getChromeLogin(myPath, userDir, filesystemObject)		#Try to get chrome passwords' database from each user (must have the masterkey for that user)
								print "Done"
		...
		for folder in folders:
			if self.harddrive in folder:	#check if harddrive is in the folder's name
				...
				for user in users:			#For each user in partition
					...
					for fi in files:
						if re.match("S-1-5-21-[0-9]+-[0-9]+-[0-9]+-[0-9]+",fi):		#User's sid is the name of the only folder in protect
	...
	def getMasterKey(self, myPath, userDir, filesystemObject):	#Tries to get the masterkey looking for the protect directory in each user folder
		...
		try:
			...
			try:
				os.removedirs(myPath+protectPath)			#If user is not valid, we delete the directory in users folder (users like 'all users', 'default' should fail)
			except:
				pass
			return False

	def getChromeLogin(self, myPath, userDir, filesystemObject):	#Tries to get the chrome passwords' database looking for the chromeLogin file
